...
const rows = computed(() => {
  const results = posts.posts.map((row, index) => ({
    index: index + 1,
    ...row
  }));
    ...
    <q-card>
      <q-card-section class="q-px-none">
        <div class="flex q-px-lg q-pb-md items-center q-gutter-x-sm">
          <q-btn color="primary" icon="o_add" label="خبر جديد" :to="{ name: 'PostsCreate' }" />
          ...
          <q-select
            ...
            :options="categoriesStore.categories['posts']"
            emit-value
            map-options
            option-value="id"
            option-label="name"
              ...
              <q-btn
                ...
                @click.stop="
                  $q.dialog({
                    ...
                    await posts.destroy(props.row.id);
                    fetch();
                  })
                "
                round
