...
categoriesStore.fetchCategories('posts');
...
const categories = computed(() => {
  const categories = JSON.parse(JSON.stringify(categoriesStore.categories.posts));
  const computedCategories = [...categories];

  categories
    .filter((e: any) => e.parent_id)
    .forEach((e: any) => {
      const parent = categories.find((x: any) => x.id == e.parent_id);
      if (!parent) {
...
async function fetch() {
  ...
  form.value = await posts.fetchDetails(props.id);
            ...
            <div class="row q-gutter-y-md">
              <div class="col-12 col-md-8">
                ...
                <q-input
                  ...
                  dense
                  v-model="form.title"
                  :rules="[(val: any) => !!val || 'يرجى ملئ الحقل بالمعلومات المناسبة']"
                />
              </div>
              <div class="col-12 col-md-8">
        ...
        <q-card>
          <q-card-section>
            <div class="row">
              ...
              <q-card class="full-width q-mt-md" bordered>
                <q-card-section>
                  <div class="text-subtitle2">التصانيف</div>
                </q-card-section>

                <q-separator dark inset />
                <q-card-section>
                  <div v-for="category in categories" :key="category.id">
                    <span class="q-ml-md" v-for="n in category.level" :key="n"></span>
                    <q-checkbox
                      :val="category.id"
                      :label="category.name"
              ...
              <div class="col-12 q-mt-md">
                <label class="label" for="tags"> كلمات مفتاحية </label>
                <q-select
                  class="q-mt-xs"
                  dense
                  outlined
